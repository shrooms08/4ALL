...
func spawn_next_chunk():
	...
	# Spawn enemies
	_spawn_chunk_enemies(new_chunk)

# NEW: Helper function to spawn enemies
func _spawn_chunk_enemies(chunk: Node2D):
...
# NEW: Helper function to cleanup chunk enemies
func _cleanup_chunk_enemies(chunk: Node2D):
	"""Clean up enemies before removing chunk"""
	var spawner = chunk.get_node_or_null("EnemySpawner")
...
func cleanup_old_chunks():
	...
	if player.global_position.y - top_chunk.global_position.y > despawn_distance:
		# Clean up enemies before freeing chunk
